...
public class EnemySpawnerN : MonoBehaviour
{
    ...
    private void Start()
    {
        WaveCheckerN.OnMaxEnemySpawn += StopSpawn;
        WaveCheckerN.OnWaveOver += StopSpawn;
        WaveCheckerN.OnWaveOver += () => Invoke(nameof(AllowSpawning), 9f);
        BossSpawner.bossSpawned += StopSpawn;
    }

    void Update()
    {
        if (!waveCheckerN.IsWaveActive()) return;
        timeTillSpawn -= Time.deltaTime;

        if (timeTillSpawn <= 0 && canSpawm)
        {
            if (waveCheckerN.GetEnemiesSpawnedThisWave() >= waveCheckerN.GetEnemiesToKillThisWave())
            {
                canSpawm = false;
    ...
    {
        float level = waveCheckerN.GetLevel(); // NEW: get level from WaveChecker
